...
####### Testing Capitalize #######
    ...
    # ("capitalize()",    "set_abcd_c",       lambda x : x.capitalize(),     lambda x : list([i.capitalize() for i in x])),
...
def test_operations(con, test_name, column, ibis_operation, expected_operation):
    ...
    if test_name == "error":
        try:
            out = test_table[getattr(test_table, column), \
                             ibis_operation(getattr(test_table, column)).name('_')].execute()
    ...
    for i, value in enumerate(original):
        ...
        if test_name == "abs()" and type(ibis_result) == float:
            ibis_result     = round(ibis_result, 2)
