...
@mutating_join_type
def test_join(how, left, s2_left, right, s2_right, df1, df2):
    ...
    # SingleStore tests:
    s2_expr = s2_left.join(s2_right, s2_left.key == s2_right.key, how=how)[
        s2_left, s2_right.other_value, s2_right.key3
    ]
    s2_result = s2_expr.execute()
    tm.assert_frame_equal(s2_result[expected.columns].sort_values(by='key').reset_index(drop=True),
                          expected.sort_values(by='key').reset_index(drop=True),
                          check_dtype=False)


def test_cross_join(left, s2_left, right, s2_right, df1, df2):
    ...
    # SingleStore tests:
    s2_expr = s2_left.cross_join(s2_right)[s2_left, s2_right.other_value, s2_right.key3]
    s2_result = s2_expr.execute()
    tm.assert_frame_equal(s2_result[expected.columns].sort_values(by='key').reset_index(drop=True),
                          expected.sort_values(by='key').reset_index(drop=True),
                          check_dtype=False)
...
@mutating_join_type
def test_join_project_left_table(how, left, s2_left, right, s2_right, df1, df2):
    ...
    # SingleStore tests:
    s2_expr = s2_left.join(s2_right, s2_left.key == s2_right.key, how=how)[s2_left, s2_right.key3]
    s2_result = s2_expr.execute()
    tm.assert_frame_equal(s2_result[expected.columns].sort_values(by='key').reset_index(drop=True),
                          expected.sort_values(by='key').reset_index(drop=True),
                          check_dtype=False)
...
def test_cross_join_project_left_table(left, s2_left, right, s2_right, df1, df2):
    ...
    # SingleStore tests:
    s2_expr = s2_left.cross_join(s2_right)[s2_left, s2_right.key3]
    s2_result = s2_expr.execute()
    tm.assert_frame_equal(s2_result[expected.columns].sort_values(by='key').reset_index(drop=True),
                          expected.sort_values(by='key').reset_index(drop=True),
                          check_dtype=False)
...
@mutating_join_type
def test_join_with_multiple_predicates(how, left, s2_left, right, s2_right, df1, df2):
    expr = left.join(
    ...
    # SingleStore tests:
    s2_expr = s2_left.join(
        s2_right, [s2_left.key == s2_right.key, s2_left.key2 == s2_right.key3], how=how
    )[s2_left, s2_right.key3, s2_right.other_value]
    s2_result = s2_expr.execute()
    print(s2_result[expected.columns])
    print(expected)
    tm.assert_frame_equal(s2_result[expected.columns].sort_values(by='key').reset_index(drop=True),
                          expected.sort_values(by='key').reset_index(drop=True),
                          check_dtype=False)
...
@mutating_join_type
def test_join_with_project_right_duplicate_column(pd_client, how, left, df1, df3):
    # also test that the order of operands in the predicate doesn't matter
    right = pd_client.table('df3')
